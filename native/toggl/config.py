...
_ENV_KEY = "VITE_TOGGL_TOKEN"


def load_token() -> str:
    """Resolve the Toggl API token.

    Order: ``TOGGL_API_TOKEN`` env, ``VITE_TOGGL_TOKEN`` env, then
    ``VITE_TOGGL_TOKEN`` from the repo-root ``.env`` so this app can reuse the
    web app's token.

    Raises:
        SystemExit: if no token can be found in any location.
    """
    token = os.environ.get("TOGGL_API_TOKEN") or os.environ.get(_ENV_KEY)
    if token:
        return token.strip()
